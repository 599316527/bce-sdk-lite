import {pick, pickBy, includes, startsWith, entries, has, assign} from 'lodash';
...
export default class BosClient extends EventBus {
    ...
    putObjectFromBlob(bucketName, key, blob, options) {
        ...
        // 浏览器会补上，不用自己写。写了反而触发 set unsafe header 报错（见 httpclient.js 里面注释）
    ...
    sendHTTPRequest(httpMethod, resource, args, config) {
        ...
        const doRequest = () => {
            ...
            promise.abort = agent.cancelRequest.bind(agent);
            return promise;
        };
